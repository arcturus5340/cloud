  ...
  <tbody>
    {% for file in files %}
        ...
        <tr  data-href="?path={{file.filepath}}{% if popup %}&amp;popup=1{% endif %}">
          ...
          <td>{{file.filesize}}</td>
          <td>{{file.filedate}}</td>
          <td align="right"><button data="{{file.filename}}" data-href="" data-url="{{file.filepath}}" class="btn btn-default link-modal-btn">{% trans "Link" %}</button></td>
          </a>
        ...
        <tr data-href="{{file.fileurl}}{% if popup %}&amp;popup=1{% endif %}">
          ...
          <td>{{file.filesize}}</td>
          <td>{{file.filedate}}</td>
          <td align="right"><button data="{{file.filename}}" data-url="{{file.filepath}}" data-href="" class="btn btn-default link-modal-btn">{% trans "Link" %}</button></td>
          </a>
            {% if not popup %}<td ><a class="file-info-button" href="#" data-href="{% url 'filemanager:detail'%}?path={{file.filepath}}"><span><i class="fa fa-info-circle"></i></span></a></td>{% endif %}
        </tr>
        {% endif %}
...
{% endif %}
 ...
 <script>
  ...
  $("#save-urls").click(function(e) {
     e.preventDefault();
    $.post("/save-urls/", {'urls' : $(".urls").val(), 'path' : $(this).attr('data'), 'csrfmiddlewaretoken': $("input[name=csrfmiddlewaretoken]").val()});
    $("#saved").show();
  });
 </script>
