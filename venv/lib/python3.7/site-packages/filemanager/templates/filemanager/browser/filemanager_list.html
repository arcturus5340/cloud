...
<br>
    <div class="row">
        ...
        <div class="col-sm-6">
          ...
          {% if request.user.is_authenticated %}<button id="create-directory-btn" data-href="{% url 'filemanager:create-directory' %}?path={{path}}{% if popup %}&amp;popup=1{% endif %}" class="btn btn-primary">Create Directory</button>
          ...
          <button id="replace-modal-btn" data-href="{% url 'filemanager:replace' %}?path={{path}}{% if popup %}&amp;popup=1{% endif %}" class="btn btn-default">{% trans "Replace" %}</button>
  ...
  <tbody>
    {% for file in files %}
        ...
        <tr  data-href="?path={{file.filepath}}{% if popup %}&amp;popup=1{% endif %}">
          ...
          <td>{{file.filesize}}</td>
          <td>{{file.filedate}}</td>
          <td align="right"><button data="{{file.filename}}" data-href="" data-url="{{file.fileurl}}" class="btn btn-default link-modal-btn">{% trans "Link" %}</button></td>
          </a>
        ...
        <tr data-href="{{file.fileurl}}{% if popup %}&amp;popup=1{% endif %}">
          ...
          <td>{{file.filesize}}</td>
          <td>{{file.filedate}}</td>
          <td align="right"><button data="{{file.filename}}" data-url="{{file.fileurl}}" data-href="" class="btn btn-default link-modal-btn">{% trans "Link" %}</button></td>
          </a>
            {% if not popup %}<td ><a class="file-info-button" href="#" data-href="{% url 'filemanager:detail'%}?path={{file.filepath}}"><span><i class="fa fa-info-circle"></i></span></a></td>{% endif %}
        </tr>
        {% endif %}
 ...
 <script>
  ...
  $("#link-access-btn").click(function() {
    if($(this).find('div.off').length !== 0) {
      $.post("/block", {'path':$(this).attr('data')});
    } else {
      $.post("/unblock", {'path':$(this).attr('data')});
    }
  });

  $("#link-url-btn").click(function() {
    if($(this).find('div.off').length !== 0) {
      $.post("/access_to_url", {'path':$(this).attr('data')});
    } else {
      $.post("/unaccess_to_url", {'path':$(this).attr('data')});
    }
  });

  $("#save-urls").click(function() {
    $.post("/save-urls", $("#url-form").serialize());
    $("#saved").show();
  });
 </script>
